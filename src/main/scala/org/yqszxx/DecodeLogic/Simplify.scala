...
object Simplify {
  def getPrimeImplicants(implicants: Seq[Term], bits: Int): List[Term] = {
    var prime = List[Term]()
  ...
  def stringify(s: Seq[Term], bits: Int): String = s.map(t => (0 until bits).map(i => if ((t.mask & (1 << i)) != 0) "x" else ((t.value >> i) & 1).toString).reduceLeft(_+_).reverse).reduceLeft(_+" + "+_)
